...
const ProjectCard = ({
  ...
  return (
    <motion.div variants={fadeIn("up", "spring", 0.5 * index, 1.75)}>
      <Tilt
      ...
      >
        <div className="relative w-full h-[230px]">
          ...
          />
          <div className=" absolute card-img_hover inset-0 flex justify-end m-3">
            <div
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {" "}
          {tags.map((tag) => (
            <p key={tag.name} className={`texr-[14px] ${tag.color}`}>
              #{tag.name}
            </p>
          ))}
        </div>
